...
def getPlanTreeJson(stu_id):
    ...
    """
    print(stu_id)
    sql = "select FINISHED from STU_PLAN WHERE STU_ID='%s'" % stu_id
    result = query(sql)
    print(result)
    ...
    sql="SELECT LEC_ID, COMMENT FROM CLASS WHERE STU_ID='%s'" % stu_id
    ...
    for co in finished_co:
        course_add = {}
        aid_str = str(aid)
        sql = "select CLASSIFICATION, START_TIME, LEC_NAME, IS_REQUIRED, CREDIT, LEC_ID from LECTURE WHERE LEC_HAS='%s'" % aid_str
        co_name = query(sql)
        #print('数据库查询结果')
        #print(co_name)
        aid = aid + 1
        ...
        if co == '0':
            ...
            add_curse['score'] = int(co2score[co_name[0][5]])#改着原5  lec_id
...
def updateDatabase(stu_id, train_plan):
    ...
    print(finish_co)
    #print(array_finish)
    sql = "UPDATE stu_plan SET FINISHED='%s' WHERE STU_ID='%s'" % (finish_co,stu_id)
    update(sql)


def updateScore(stu_id, scores):
    sql="SELECT LEC_ID, LEC_NAME FROM LECTURE"
    name2no = {}
    result = query(sql)
    for cur in result:
        name2no[cur[1]] = cur[0]
    ...
    for cur in scores:
        sql="UPDATE CLASS SET COMMENT='%d' WHERE STU_ID='%s' AND LEC_ID='%s'" % (scores[cur], stu_id, name2no[cur])
        #print(sql)
        update(sql)